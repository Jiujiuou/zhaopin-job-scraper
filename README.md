...
批量抓取智联招聘岗位，按薪资 / 城市 / 公司黑名单 / 岗位类型过滤，生成**可勾选、可批量投递的投递工作台报告**。
...
- **投递工作台**：紧凑网格一屏多卡，整卡点击勾选，勾选状态自动保存，AI 可直接批量投递
...
完成后当前目录会出现 `智联_嵌入式开发_深圳_薪资15K.html` —— 这是一个**投递工作台**：
- 浏览器打开后，网格布局一屏显示 30+ 岗位，整卡点击即可勾选/取消
- 顶部工具栏支持全选 / 反选 / 按薪资排序 / 按 HR 活跃度排序 / 仅看已选
- 勾选状态自动保存到浏览器 `localStorage`，AI 可直接读取并批量投递
...
```bash
# 启动本地服务器（file:// 下 AI 无法访问，必须用 http://）
cd 报告目录 && python -m http.server 8765

# AI 读取你勾选的清单（含每个岗位的详情 URL）
# AI 逐个打开详情页 → 点击「立即投递」→ 验证按钮变「已投递」
```

> 详见 [SKILL.md](SKILL.md) 第 5 步投递工作流与踩坑记录。
...
**Q: AI 投递时点不动「立即投递」按钮？**
A: bsk 的 `browser-skill-overlay` 透明层会拦截坐标点击。必须用 `bsk evaluate` 执行 `document.querySelector('button.a-button.a--bordered.a--filled').click()`，不要用坐标点击。

**Q: AI 读不到我勾选的岗位？**
A: 报告必须通过 `http://127.0.0.1:8765/<文件名>.html` 打开（不能双击 file:// 打开），且勾选后 localStorage 才会保存。换地址后需要重新勾选一次。

**Q: 投递多了会被智联限制吗？**
A: 建议单次投递 ≤30 个，间隔 5-10 秒，分批次进行。遇到验证码立即停下人工处理，不要硬刚。

## 📌 合规声明

- 本工具**仅用于个人求职**，使用你自己的浏览器登录态，不绕过任何验证机制
- 脚本内置 1-3 秒抓取间隔，投递间隔 5-10 秒，请勿调小，尊重智联服务条款
...
**最后验证日期**：2026-08-13（智联页面结构 + 260 城市代码 + 前端/嵌入式/运营三画像实测 + 投递端到端实测）
